...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    leclient = clientTCP()
    # [M]ensaje [2]Alajuela [7]SanRamon [32]Clinica??? [Datos]
    msg = leclient.encode_data_vac('M', 2, 71, 321, 'Stormblessed')
    s.connect((HOST, PORT))
    s.send(msg.encode('utf-8'))
    #s.sendall(b'Informacion')
    data = s.recv(1024)
    print('Received', repr(data)),
    leclient.process_message(data)